      ...
      <section className="about" id="about">
        <div className="container">
          ...
          <p className="section__subtitle">
            Целеустремленный, ответственный и исполнительный. Имея аналитический
            склад ума и логическое мышление - в процессе верстки продумываю
            каждую деталь. Верстка сайтов, интернет-магазинов и Landing Page
            (фиксированная, резиновая, адаптивная). Работаю удаленно. В работе
            использую SCSS, БЕМ, Gulp, Jquery, Javascript, Git, CMS Wordpress,
            React, NodeJs, Express, MongoDB, Redux.
          </p>
        </div>
      </section>
          ...
          <div className="expert__cards">
            {technologyItems.length &&
              technologyItems.map((item) => (
                <TechnologyItem key={item.id} {...item} />
              ))}
          </div>
        </div>
      </section>
      ...
      <section className="works">
        <div className="container">
          ...
          <div className="works__inner">
            {workItems.length &&
              workItems.map((item) => <Cart key={item.id} {...item} />)}
          </div>
        </div>
      </section>
